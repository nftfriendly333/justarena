[iron-arena.html](https://github.com/user-attachments/files/26580392/iron-arena.html)
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
function updateFightButtonWalletGate() {
  // Wallet is set automatically from MetaMask login — always enable fight button
...
function deleteSave() {
  ...
  try { const key = await _arenaStorageKey(); if (key && window.storage) await window.storage.delete(key, false); } catch {}
  localStorage.removeItem('ironArena_guildName');
  localStorage.removeItem('ironArena_guildLeader');
...
(async () => {
  try {
    ...
    // Set wallet from MetaMask address
    if (profile.address) {
